...
pub(crate) fn new_session_info(
    ...
    if is_first_event {
        ...
        // Re‑insert non‑duplicate items from the original initializer
        lines.push(Line::from(vec![
            " /fork".bold(),
            format!(" - {}", SlashCommand::Fork.description()).dim(),
        ]));
        lines.push(Line::from(vec![
            " /hfork".bold(),
            format!(" - {}", SlashCommand::Hfork.description()).dim(),
        ]));
        lines.push(Line::from(vec![
            " /vfork".bold(),
            format!(" - {}", SlashCommand::Vfork.description()).dim(),
        ]));
